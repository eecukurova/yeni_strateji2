...
class Strategy:
    def __init__(self, timeframe=None):
        ...
        # Bad signal tracking for score filter
        self.bad_rsi_signals = []
        self.bad_atr_signals = []
    ...
    def check_market_conditions(self, df):
        """Market koşullarını kontrol eder"""
        if len(df) < 200:  # Yeterli veri yoksa
            return False
        ...
        volatility_ok = self.config.min_atr < atr_value < self.config.max_atr
        
        # RSI kontrolü
        rsi_value = last_row.get('rsi', 50)
        
        # Trend kontrolü (EMA 50 > EMA 200)
        ema50 = last_row.get('ema_50', 0)
        ema200 = last_row.get('ema_200', 0)
        trend_ok = ema50 > ema200
        
        # Bad trade rate kontrolü (simüle edilmiş)
        # Gerçek uygulamada bu değer trade history'den hesaplanmalı
        bad_trade_ok = True  # Şimdilik True olarak ayarlandı
        
        # Market koşulları uygun mu?
        market_conditions_ok = bad_trade_ok  # trend_ok da eklenebilir
        
        return market_conditions_ok

    def check_score_filter(self, df):
        """Score filter kontrolü"""
        if len(df) < 1:
            return True
        ...
        # Bad signal listelerini kontrol et
        def is_close_to_bad_signal(value, bad_list, tolerance):
            for bad_value in bad_list:
                if abs(value - bad_value) < tolerance:
                    return True
            return False
        
        # RSI ve ATR geçmiş kötü sinyallere yakınsa engelle
        score_filter_ok = not (
            is_close_to_bad_signal(rsi_value, self.bad_rsi_signals, self.config.bad_signal_tolerance) or
            is_close_to_bad_signal(atr_value, self.bad_atr_signals, self.config.bad_signal_tolerance)
        )
        
        return score_filter_ok

    def add_bad_signal(self, rsi_value, atr_value):
        """Zararlı işlem sinyallerini kaydet"""
        self.bad_rsi_signals.insert(0, rsi_value)
        self.bad_atr_signals.insert(0, atr_value)
        
        # Maksimum kayıt sayısını kontrol et
        if len(self.bad_rsi_signals) > self.config.max_bad_signals:
            self.bad_rsi_signals.pop()
        if len(self.bad_atr_signals) > self.config.max_bad_signals:
            self.bad_atr_signals.pop()
    ...
    def determine_position(self, df):
        ...
        # Trend EMA'ları
        df['ema_50'] = self.calculate_ema(df, self.config.ema_trend_short)
        df['ema_200'] = self.calculate_ema(df, self.config.ema_trend_long)
        
        # ATR ve RSI hesapla
        df['atr'] = self.calculate_atr(df, self.config.atr_length)
        df['rsi'] = self.calculate_rsi(df, self.config.rsi_length)

        # Alım-satım sinyalleri (Pine Script mantığı)
        df['buy_signal'] = (
            (df['zone_decider'] == 1) & 
            (df['zone_decider'].shift(1) == -1) & 
            (df['Close'] > df['middle_donchian'])
        )
        ...
        # Market condition ve score filter kontrolleri
        df['market_conditions_ok'] = df.apply(lambda row: self.check_market_conditions(df.loc[:df.index.get_loc(row.name)]), axis=1)
        df['score_filter_ok'] = df.apply(lambda row: self.check_score_filter(df.loc[:df.index.get_loc(row.name)]), axis=1)
        
        # Final trade koşulları
        df['can_trade'] = df['market_conditions_ok'] & df['score_filter_ok']
        
        # Final sinyaller
        df['buy'] = df['buy_signal'] & df['can_trade']
        df['sell'] = df['sell_signal'] & df['can_trade']
